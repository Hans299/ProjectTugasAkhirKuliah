            ...
            <div class="search-bar" style="max-width: 400px;">
                <form method="GET" id="search-form">
                    <input type="text" name="search" id="search-input" value="{{ request('search') }}"
                        class="form-control" placeholder="Cari Judul Buku" autocomplete="off">
                </form>
            ...
            <table class="table table-striped table-hover">
                ...
                <tbody>
                    ...
                    @forelse($bukus as $buku)
                        <tr>
                            <td style="text-align: left;">{{ $buku->judul_buku }}</td>
                            <td>{{ $buku->isbn }}</td>
                            <td>{{ $buku->penerbit ?? '-' }}</td>
                            ...
                            <td class="action-buttons">
                                ...
                                <a href="{{ route('admin.pustakawan.buku.edit', $buku->id) }}"
                                    class="btn btn-warning text-white">
                                    <i class="fa fa-pencil-alt"></i>
                                </a>

                                {{-- Tombol Hapus (Langkah 45) --}}
                                <form action="{{ route('admin.pustakawan.buku.destroy', $buku->id) }}" method="POST"
                                ...
                                <a href="{{ route('admin.pustakawan.buku.show', $buku->id) }}"
