...
@section('content')
    ...
    <h2 class="page-title mb-4">
        <a href="{{ route('admin.laboran.alat.index') }}" class="breadcrumb-link">Labotarium</a> > Edit Alat Lab
    </h2>
    ...
    <div class="form-card">
        ...
        --}}
        <form action="{{ route('admin.laboran.alat.update', $alat->id) }}" method="POST" enctype="multipart/form-data">
            <div class="mb-3  d-flex flex-column align-items-center">
                @if ($alat->gambar)
                    <img src="{{ asset('storage/' . $alat->gambar) }}" alt="Alat" class="current-image d-block">
                @else
                ...
                <div class="col-lg-6">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="nama_alat" name="nama_alat"
                            placeholder="Masukan Nama Alat" value="{{ old('nama_alat', $alat->nama_alat) }}" required>
                        @error('nama_alat')
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="id_alat" name="id_alat"
                            placeholder="Masukan ID Alat" value="{{ old('id_alat', $alat->id_alat) }}" required>
                        @error('id_alat')
                            <div class="text-danger mt-1">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="col-lg-6">
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="kualitas" name="kualitas" required>
                            ...
                            <option value="Sangat Baik"
                                {{ old('kualitas', $alat->kualitas) == 'Sangat Baik' ? 'selected' : '' }}>Sangat Baik
                            </option>
                            <option value="Baik" {{ old('kualitas', $alat->kualitas) == 'Baik' ? 'selected' : '' }}>Baik
                            </option>
                            <option value="Buruk" {{ old('kualitas', $alat->kualitas) == 'Buruk' ? 'selected' : '' }}>
                                Buruk
                            </option>
                        </select>
                        @error('kualitas')
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control" id="stok" name="stok"
                            placeholder="Masukan Stok Alat" value="{{ old('stok', $alat->stok) }}" required>
                        @error('stok')
                            <div class="text-danger mt-1">{{ $message }}</div>
                        @enderror
                    </div>
